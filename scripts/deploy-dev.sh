#!/usr/bin/env bash
# 랜딩 페이지(HP-87)를 landing.replix-dev.site 에 올린다 — 팀이 머지 전 렌더링된 화면을
# diff 없이 확인하고 싶을 때 쓰는 **임시 미리보기**다(이미지 빌드 → ECR push → 롤아웃 → 검증).
#
# ⚠️ 2026-08-06 결정: 이 배포는 어떤 브랜치에도 자동 연결되지 않는다 — develop 같은 영구
# dev 브랜치는 두지 않는다(정적 페이지라 백엔드식 위험 격리가 필요 없다는 판단). 지금 로컬에
# 체크아웃돼 있는 걸 그대로 올리는 것뿐이므로, 보여주고 싶은 브랜치로 바꾼 뒤 이 스크립트를
# 돌리면 된다. 실제 배포(replix.tv)는 이 경로를 거치지 않는다 — main 머지 시 GitHub Pages 가
# docs/ 를 직접 서빙한다(레포 루트 README 참조).
#
# 사용법:
